...
/* Subroutine */int dicfs_(size_t *n, size_t *nnz, double *a, double *adiag,
		...
		size_t *iwa, double * wa1, double *wa2)
{
	...
	/* Local variables */
	static ptrdiff_t i__, j;
	static size_t k, nb;
	static ptrdiff_t info;
	...
	/*       std::min<double>PACK-2  ......  dicf */

	/*     std::min<double>PACK-2 Project. October 1998. */
	/*     Argonne National Laboratory. */
...
#endif
	for (i__ = 1; i__ <= (ptrdiff_t)i__1; ++i__)
	{
		/* Computing 2nd power */
		wa1[i__] = adiag[i__] * adiag[i__];
	}
	i__1 = *n;
	for (j = 1; j <= (ptrdiff_t)i__1; ++j)
	{
		i__2 = acol_ptr__[j + 1] - 1;
		for (i__ = acol_ptr__[j]; i__ <= (ptrdiff_t)i__2; ++i__)
		{
			k = arow_ind__[i__];
			/* Computing 2nd power */
			d__1 = a[i__] * a[i__];
...
#pragma omp parallel for private(i__)
	...
	*alpha = 0.;
	...
	while (m_true)
	{
		...
		i__1 = *n + 1;
		memcpy(&lcol_ptr__[1], &acol_ptr__[1], sizeof(size_t) * i__1);
		//std::copy(&acol_ptr__[1], &acol_ptr__[i__1 + 1], &lcol_ptr__[1]);
		i__1 = *nnz;
		memcpy(&lrow_ind__[1], &arow_ind__[1], sizeof(size_t) * i__1);
...
#pragma omp parallel for private(i__)
#endif
			for (i__ = acol_ptr__[j]; i__ <= (ptrdiff_t)(acol_ptr__[j + 1] - 1); ++i__)
			{
		...
		/*        If the factorization exists, then test for terstd::min<double>ation. */
		...
		size_t *indf, size_t *list, double *w)
{
	...
	/* Local variables */
	static size_t i__, ip, np, iej, iek, mlj, nlj, isj, kth, isk;
	static ptrdiff_t j, k;
	static double lval;
	static size_t newk;
	static size_t newiej, newisj;
	...
	/*       std::min<double>PACK-2  ......  dsel2, ihsort, insort */

	/*       Level 1 BLAS  ...  daxpy, dcopy, ddot, dnrm2 */

	/*     std::min<double>PACK-2 Project. May 1998. */
	/*     Argonne National Laboratory. */
	/*     Chih-Jen Lin and Jorge J. More'. */
	...
	*info = 0;
	i__1 = *n;
	memset(&indf[1], 0, sizeof(size_t) * i__1);
	memset(&list[1], 0, sizeof(size_t) * i__1);
...
#endif
		for (k = 1; k <= (ptrdiff_t)i__2; ++k)
		{
			w[indr[k]] /= diag[j];
		}
		/*        Set mlj to the number of nonzeros to be retained. */
		/* Computing std::min<double> */
		i__2 = iej - isj + 1 + *p;
		mlj = std::min<size_t>(i__2, nlj);
		kth = nlj - mlj + 1;
		if (nlj >= 1)
		{
...
#pragma omp parallel for private(k)
#endif
		for (k = newisj; k <= (ptrdiff_t)i__2; ++k)
		{
...
int dsel2_(size_t *n, double *x, size_t *keys, size_t *k)
{
	...
	/* Local variables */
	static size_t i__, l, m, p, u, p1, p2, p3, lc, lp/*, swap*/;
	...
	/*     std::min<double>PACK-2 Project. March 1998. */
	/*     Argonne National Laboratory. */
...
{
	static size_t k, m, x, mid, lheap, rheap;
	...
	/*     This subroutine is a std::min<double>or modification of code written by */
...
#pragma omp parallel for private(k)
#endif
			for (k = jptr[j]; k <= (ptrdiff_t)i__2; ++k)
			{
				r__[indr[k]] -= l[k] * temp;
			}
			r__[j] = temp;
...
#endif
			for (k = jptr[j]; k <= (ptrdiff_t)i__1; ++k)
			{
				temp += l[k] * r__[indr[k]];
			}
